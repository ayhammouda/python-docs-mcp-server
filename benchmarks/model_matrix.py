...
``benchmarks.runner``. Wiring model-matrix cells into the runner's per-cell
execution path (so a manifest competitor can be paired with a specific model
family end to end) is deferred to a follow-up issue once the maintainer
confirms how that composition should affect the artifact shapes already
covered by ``tests/benchmarks/test_runner.py`` -- this package only defines
the matrix, the cross-multiplication contract, and mocked provider adapters
(see ``benchmarks.adapters``).
"""
